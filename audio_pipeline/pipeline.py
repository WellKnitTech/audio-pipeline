...
def run_pipeline(input_dir: Path, output_root: Path, config: PipelineConfig) -> None:
    ...
    stages_per_file = sum(
        [
            1 if config.features.enhance else 0,
            1 if (config.features.make_asr_wav or config.features.diarize) else 0,
            1 if config.features.transcribe else 0,
            1 if config.features.diarize else 0,
        ]
    )

    with tqdm(total=len(files), desc="Files", unit="file") as files_progress:
        for in_file in files:
            ...
            with tqdm(
                ...
                if config.features.enhance:
                    ...
                    file_progress.set_postfix_str("enhance")
                ...
                if config.features.make_asr_wav or config.features.diarize:
                    ...
                    file_progress.set_postfix_str("make_asr_wav")
                ...
                if config.features.transcribe:
                    file_progress.set_postfix_str("transcribe")
                ...
                if config.features.diarize and diarizer is not None:
                    file_progress.set_postfix_str("diarize")
